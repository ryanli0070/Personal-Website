...
function ExternalArrow() {
  return (
    <svg
      className="w-4 h-4 text-neutral-600 group-hover:text-neutral-400 transition-colors ml-auto shrink-0"
      fill="none"
      viewBox="0 0 24 24"
      stroke="currentColor"
      strokeWidth={2}
    ...
    <section className="min-h-screen flex flex-col items-center justify-center px-6 py-28">
      <div className="max-w-lg w-full text-center">
        ...
        <div className="space-y-4">
          {contacts.map(({ label, href, sublabel, icon }) => (
            <a
              key={label}
              href={href}
              target="_blank"
              rel="noopener noreferrer"
              className="group flex items-center gap-4 rounded-xl border border-white/10 bg-white/[0.03] px-6 py-4 transition-all duration-300 hover:border-white/20 hover:bg-white/[0.06]"
            >
